toLatexTable = function(ANOVA, EF, fixed.names) {
    ...
    match.fixed.names =  unique(unlist(strsplit(fixed.ColNames[1:(length(fixed.ColNames)/2)], "[[:punct:]]")))
    fixed.names =  fixed.names[1:length(match.fixed.names)]
    match.fixed.names = c(match.fixed.names, ":", "*")
    fixed.names = c(fixed.names,  ":", "*")
    
     fixed.ColNames = 
          sapply(strsplit(gsub("([[:punct:]])", "\\.\\1\\.", fixed.ColNames[1:(length(fixed.ColNames)/2)]), "\\."),
    ...
    # check for interaction effects    
     random.names = 
          sapply(strsplit(gsub("([[:punct:]])", "\\.\\1\\.", random.ColNames[startColNames:ColNamesLen]), "\\."),
    ...
    firstRow = rbind(firstRow, paste(paste("\\multicolumn{1}{l}{$\\bm{E_{", fixed.names, sep = "", collapse = "}}$}&"), "}}$}\\\\ \n", sep = ""))
    ...
    for (i in 1:length(matchRowNames)) {
        ...
        total.VC = gsub("\\$\\$", "", total.VC)
         ...
         if (length(grep("MS", random.ColNames)) == 1) {
            eff = paste("$", finalTable[i, (3 + length(random.names) + length(fixed.names)):ncol(finalTable)], "$", sep = "", collapse = " & ")
         } else {
        ...
        if (length(grep("MS", random.ColNames)) == 1) {
            currentRow = paste(SV, " & ", DF, " & ", total.VC, " & ", finalTable[i, 2 + length(random.names)],  " &", eff, "\\\\", sep = "")
        } else {
            currentRow = paste(SV, " & ", DF, " & ", total.VC, " &", eff, "\\\\", sep = "")
        }
        
        if((grepl("Between", matchRowNames[i + 1]) || grepl("Within", matchRowNames[i + 1])) && 
        !grepl("^Within", matchRowNames[i]) && !grepl("^Between", matchRowNames[i])){ 
