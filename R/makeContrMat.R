...
#' described by John and Williams (1987). These C matrics are used for the
#' information decomposition for every treatment effect in every stratum of the
#' experiment.
#' 
...
#' for the block effects of Phase 1 experiment can also be contructed using
#' this function.
#' 
...
#' @param contr.vec a list of contrast vectors, this allows the user to specify
#' the contrasts for each treatment or block factor. Note that if this
#' arguement is used, it is necessary to specify the contrasts for every
#' treatment or block factor with the same order as \code{effectNames}. Default
#' is \code{NA}, and the function output the C matrices described by John and
#' Williams (1987).
