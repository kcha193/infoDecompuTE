makeContrMat <- function(design.df, effectNames, effectsMatrix, contr.vec) {
    ...
    if (all(is.na(contr.vec))) {
		...
		for(i in 1:length(new.contr.vec)){
			new.contr.vec[i] = ifelse(is.null(contr.vec[[names(new.contr.vec)[i]]]), NA, contr.vec[names(new.contr.vec)[i]])
		}
            ...
            if (any(grepl("[[:punct:]]", effectNames))) {
                ...
                for (i in interTerms) {
                  ...
                  new.contr.vec[[i]] = vector(length = nlevels(interaction(mainTerms.list)), mode = "list")
                ...
                if (is.list(contr.vec[[rownames(effectsMatrix)[i]]])) {
                  ...
                  for (j in 1:ncol(newEffectsMatrix)) {
                    ...
                    if (newEffectsMatrix[i, j] == 1) {
                      trtNames = match(unlist(strsplit(colnames(newEffectsMatrix)[j], "\\.")), names(trtContr))
