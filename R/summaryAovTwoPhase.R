...
#' experimetnal design with the structural formulae of the block and treatment
#' factors. Two tables containing the variance components of the random effects
...
#' factor as the variance components with the first phase's variance components
#' in \code{random.terms1} appear before the second phase's variance components
#' in \code{random.terms2}.
...
#' experiment. Note that if this arguement is used, it is necessary to specify
...
summaryAovTwoPhase <- function(design.df, blk.str1, blk.str2, trt.str, var.comp = NA, 
    ...
    if (any(grepl(":", blkTerm2))) {
        ...
        for (i in 1:length(split.check.blkTerm2)) {
            ...
            newName <- paste(newName, split.check.blkTerm2[[i]][length(split.check.blkTerm2[[i]])], 
                sep = "")
        ...
        if (length(len) > 0) {
            for (i in 1:length(len)) check.blkTerm2[len[i]] <- checkBrack(check.blkTerm2[len[i]], 
                blk.str2)
    ...
    if (any(grepl(":", blkTerm1))) {
        ...
        for (i in 1:length(split.check.blkTerm1)) {
            ...
            newName <- paste(newName, split.check.blkTerm1[[i]][length(split.check.blkTerm1[[i]])], 
                sep = "")
        ...
        if (length(len) > 0) {
            for (i in 1:length(len)) check.blkTerm1[len[i]] <- checkBrack(check.blkTerm1[len[i]], 
                blk.str1)
