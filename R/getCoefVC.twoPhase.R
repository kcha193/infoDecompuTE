...
function(Pb, design.df, v.mat, response, table.legend){
   ...
   if(all(is.na(response))){
    ...
    MS = lapply(Pb, function(x) lapply(x, function(q) lapply(q, function(w) t(response) %*% w %*% response)))
	...
	for(i in 1:length(Pb)){
        ...
        noindent = TRUE
			...
			for(k in 1:(length(Pb[[i]]))){
				tmp <- matrix(0, nrow=length(names(Pb[[i]][[k]])) , ncol=(length(V) + 1),
						dimnames=list(names(Pb[[i]][[k]]), c(names(V), "MS")))
				...
				if(nrow(tmp) == 1 && grepl("Residual", rownames(tmp))){
					...
					tmp = c(attr(fractions(tmp[-length(tmp)]),"fracs"), as.character(round(tmp[length(tmp)], digits = 10)))
					VC = rbind(VC, tmp)
